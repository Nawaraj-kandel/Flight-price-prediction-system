...
const Profile = () => {
  ...
  const handleCancelBooking = async () => {
    ...
    setCancelLoading(true);  // Show loading indicator during cancellation
    try {
      const authToken = getAuthToken();
      const response = await fetch(`${API_BASE_URL}/flight/cancel?booking_id=${selectedBookingId}`, {
        method: "POST",
      ...
      setCancelLoading(false);  // Hide loading indicator
      setSelectedBookingId(null);
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg text-center">
            ...
            <div className="flex justify-center space-x-4 mt-4">
              ...
              <button
                onClick={handleCancelBooking}
                className="px-4 py-2 bg-red-500 text-white rounded-lg"
                disabled={cancelLoading}  // Disable button during loading
              >
